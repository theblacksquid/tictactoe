...
(function() {
  ...
  game.main = function(parent, num) {
    var cell, grd, isState, j, len1, ref, results, row;
  ...
  ai["eval"] = function(input) {
    var j, k, l, len1, len2, len3, point, pos, pt, ref, surround;
    ref = input.corners;
    for (j = 0, len1 = ref.length; j < len1; j++) {
      point = ref[j];
      if (point.status === 'unclaimed') {
        return point.pos;
      }
    }
    if (getRandInt(1, 2) === 1) {
  ...
  ai.respond = function() {
    ...
    if (game.players.ai === "X") {
      if ((game.turns % 2) !== 0) {
        ...
        return $("cell-" + (getX(choose)) + "-" + (getY(choose)) + "-img").off();
      }
