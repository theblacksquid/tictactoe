...
(function() {
  ...
  ai["eval"] = function(input) {
    var coin_toss, get_rand, key, pt;
    pt = input.points.self[getRandInt(0, input.points.self.length)];
    coin_toss = getRandInt(1, 2);
    for (key in input.corners) {
      if (key.status === 'unclaimed') {
        return value.pos;
      }
    }
    get_rand = function() {
      return [getRandInt(0, input.length), getRandInt(0, input.length)];
    };
    return (function() {
      var j, k, len1, len2, pos, surround;
      if (pt === void 0) {
        return get_rand();
      } else if (coin_toss === 1) {
        surround = ptsAroundpt(pt);
        for (j = 0, len1 = surround.length; j < len1; j++) {
          pos = surround[j];
          if (pos === void 0) {
            break;
          }
